...
static gchar *show_admin_address(void *ex_param);
static gint assign_admin_username(const char *newval, void *ex_param);
...
NETWORK_MYSQLD_PLUGIN_PROTO(server_read_auth) {
    ...
    g_rw_lock_reader_lock(&con->srv->backends->au->admin_user_lock);

    /* for admin user */
    if (admin_user_host_check(con->srv->backends->au, NETWORK_SOCKET_SRC_NAME(con->client))) {
            network_mysqld_con_send_error_full_nolog(send_sock, C("unpermitted host"), 1045, "28000");
            con->state = CON_STATE_SEND_ERROR; /* close the connection after we have sent this packet */
    } else if (!strleq(S(con->client->response->username), con->srv->backends->au->name, strlen(con->srv->backends->au->name))) {
        network_mysqld_con_send_error_full_nolog(send_sock, C("unknown user"), 1045, "28000");
        
        con->state = CON_STATE_SEND_ERROR; /* close the connection after we have sent this packet */
    } else if (network_mysqld_proto_password_hash(hashed_password, con->srv->backends->au->password, strlen(con->srv->backends->au->password))) {
    } else if (network_mysqld_proto_password_scramble(excepted_response,
                S(recv_sock->challenge->challenge),
                S(hashed_password))) {
        network_mysqld_con_send_error_full_nolog(send_sock, C("scrambling failed"), 1045, "28000");
        SEND_ERR_MSG_HANDLE(g_critical, "scrambling failed", send_sock);
        con->state = CON_STATE_SEND_ERROR; /* close the connection after we have sent this packet */
    } else if (!g_string_equal(excepted_response, auth->response)) {
        network_mysqld_con_send_error_full_nolog(send_sock, C("password doesn't match"), 1045, "28000");
        SEND_ERR_MSG_HANDLE(g_critical, "password doesn't match", send_sock);
        con->state = CON_STATE_SEND_ERROR; /* close the connection after we have sent this packet */
    } else {
...
static network_mysqld_lua_stmt_ret admin_lua_read_query(network_mysqld_con *con) {
    ...
    if (COM_QUERY == command) {
        ...
        if (packet->len - NET_HEADER_SIZE - 1 >= sizeof("LOAD ") - 1 &&
            0 == g_ascii_strncasecmp(packet->str + NET_HEADER_SIZE + 1, C("LOAD ")) &&
            (g_strrstr (packet->str + NET_HEADER_SIZE + 1, "INFILE") ||
                   g_strrstr (packet->str + NET_HEADER_SIZE + 1, "infile")))
        {
            g_log_dbproxy(g_critical, "%s", "[admin][foribidden load]");
 ...
 */
static chassis_options_t * network_mysqld_admin_plugin_get_options(chassis_plugin_config *config) {
    if (config->opts == NULL) {

        chassis_options_t *opts = chassis_options_new();
        chassis_options_add(opts, "admin-address",            0, 0, G_OPTION_ARG_STRING, &(config->address), "listening address:port of the admin-server (default: :4041)", "<host:port>", NULL,
                            show_admin_address, SHOW_OPTS_PROPERTY);
        chassis_options_add(opts, "admin-username",           0, 0, G_OPTION_ARG_STRING, &(config->admin_username), "username to allow to log in", "<string>",
                            assign_admin_username, show_admin_username, ALL_OPTS_PROPERTY);
        chassis_options_add(opts, "admin-password",           0, 0, G_OPTION_ARG_STRING, &(config->admin_password), "password to allow to log in", "<string>",
                            assign_admin_password, show_admin_password, ALL_OPTS_PROPERTY);
    chassis_options_add(opts, "admin-user-hosts", 0, 0, G_OPTION_ARG_STRING_ARRAY, &(config->admin_hosts), "host to allow to log in", "<string>", NULL, NULL, ALL_OPTS_PROPERTY);
        config->opts = opts;
    }
...
static int network_mysqld_admin_plugin_apply_config(chassis *chas, chassis_plugin_config *config) {
    ...
    if (!config->address) {
        g_log_dbproxy(g_critical, "%s", "Failed to get bind address, please set by --admin-address=<host:port>");
        return -1;
    }
    if (!config->admin_username) {
        g_log_dbproxy(g_critical, "%s", "--admin-username needs to be set");
        return -1;
    }
    if (!config->admin_password) {
        g_log_dbproxy(g_critical, "%s", "--admin-password needs to be set");
        return -1;
    }
    if (!config->lua_script) {
        ...
        admin_user_user_update(chas->backends->au, config->admin_username, config->admin_password);
    for (i = 0; config->admin_hosts && config->admin_hosts[i]; i++) {
        gchar *cur_user_info = g_strdup(config->admin_hosts[i]);
        gchar *tmp_for_free = cur_user_info;

        cur_user_info = g_strstrip(cur_user_info);

        admin_user_host_add(chas->backends->au, cur_user_info);
        g_free(tmp_for_free);
    }   
    /** 
...
static gchar *
show_admin_address(void *ex_param)
{
    return g_strdup(config->address);
}

static gint
assign_admin_username(const char *newval, void *ex_param)
{
    external_param *opt_param = (external_param *) ex_param;
    chassis *srv = opt_param->chas;
    admin_user_info *au = srv->backends->au;
    ...
    au->name = g_strdup(newval);
    g_rw_lock_writer_unlock(&au->admin_user_lock);

    return 0;
}
static gchar *
show_admin_username(void *ex_param)
{
    ...
    g_rw_lock_reader_lock(&au->admin_user_lock);
    admin_username = g_strdup(au->name);
    g_rw_lock_reader_unlock(&au->admin_user_lock);

    return admin_username;
}

static gint
assign_admin_password(const char *newval, void *ex_param)
{
    external_param *opt_param = (external_param *) ex_param;
    chassis *srv = opt_param->chas;
    admin_user_info *au = srv->backends->au;
...
static gchar *
show_admin_password(void *ex_param)
{
    gchar *admin_password = NULL;
    external_param *opt_param = (external_param *) ex_param;
    chassis *srv = opt_param->chas;
    admin_user_info *au = srv->backends->au;

    g_rw_lock_reader_lock(&au->admin_user_lock);
    if (opt_param->opt_type == SAVE_OPTS) {
        admin_password = g_strdup(au->password);
    } else {
        admin_password = encrypt_dbproxy(au->password);
    }
    g_rw_lock_reader_unlock(&au->admin_user_lock);
