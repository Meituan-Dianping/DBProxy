 ...
 */
static chassis_options_t * network_mysqld_debug_plugin_get_options(chassis_plugin_config *config) {
    if (config->opts) {
        chassis_options_t *opts = chassis_options_new();
        chassis_options_add(opts, "debug-address",            0, 0, G_OPTION_ARG_STRING, &(config->address), "listening address:port of the debug-server (default: :4043)", "<host:port>",
                            NULL, NULL, ALL_OPTS_PROPERTY);
