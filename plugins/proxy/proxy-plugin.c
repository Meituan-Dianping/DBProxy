...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_read_handshake) {
    ...
    if (err)
    {
        CON_MSG_HANDLE(g_critical, con, "read hand shake's network header failed");
    ...
    if (err)
    {
        CON_MSG_HANDLE(g_critical, con, "read hand shake's execute result failed");
        return NETWORK_SOCKET_ERROR;
    }
    ...
    if (status == 0xff) {
        ...
        if (err) {
            CON_MSG_HANDLE(g_critical, con, "read hand shake get errcode failed");
        }
        if (packet.offset < packet.data->len) {
            err = err || network_mysqld_proto_get_string_len(&packet, &errmsg, packet.data->len - packet.offset);
            if (err) {
                CON_MSG_HANDLE(g_critical, con, "get handshake packets' errmsg failed ");
            }
        }
...
//  g_string_assign_len(con->client->default_db, S(auth->database));
    ...
    if (!check_res) {
        ...
        SEND_ERR_MSG_HANDLE(g_critical, error->str, recv_sock);
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_send_query_result) {
    ...
    if (!inj->resultset_is_needed && st->injected.sent_resultset > 0) {
        ...
        g_log_dbproxy(g_critical, "proxy.queries:append() in %s can only have one injected query without { resultset_is_needed = true } set. We close the client connection now.", config->lua_script);
...
static void check_backend_thread_running(network_backend_t* backend, MYSQL *mysql) {
    ...
    if (mysql_query(mysql, "show status like 'Threads_running'")) {
        g_log_dbproxy(g_critical, "get backend (%s) threads_running failed:%s", backend->addr->name->str, mysql_error(mysql));
        return;
    }
    ...
    if (result != NULL) {
        if (mysql_num_fields(result) > 1 && mysql_num_rows(result) > 0) {
            ...
            g_log_dbproxy(g_critical, "get backend (%s) threads_running failed, num_fields:%d, num_rows:%d", backend->addr->name->str, mysql_num_fields(result), mysql_num_rows(result));
        }
    ...
    while (!chassis_is_shutdown()) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if ((monitor_user == NULL && mysql_errno(&mysql) == ER_ACCESS_DENIED_ERROR) ||
                ...
                g_log_dbproxy(g_warning, "set backend(%s) state to DOWN for: %d(%s)",
...
query :
            ...
            if (result == NULL) {
                if (j++ < config->check_state_retry_times) {
                    ...
                    bt = BACKEND_STATE_DOWN;
                    g_log_dbproxy(g_warning, "set backend(%s) state to DOWN for retry %d times to get thread_running",
                ...
                if (backend->thread_running < chas->max_backend_tr) {
                    ...
                    if (k++ < config->check_state_retry_times) {
                        ...
                        g_log_dbproxy(g_warning, "set backend(%s) to PENDING due to thread running is %d",
...
set_state:
            ...
            if (backend->state != bt) {
                ...
                //g_log_dbproxy(g_message, "set backend (%s) state to %s",//重复
                //        backend->addr->name->str,
...
int network_mysqld_proxy_plugin_apply_config(chassis *chas, chassis_plugin_config *oldconfig) {
    ...
    for (i = 0; config->user_ips_str && config->user_ips_str[i]; i++) {
        ...
        if (is_complete) {
            if (user_hosts_handle(bs, user, ips, ADD_USER_HOST) != 0) {
                g_log_dbproxy(g_critical, "add user hosts failed: user %s doesn't exist", user);
            }
        }
    ...
    for (i = 0; config->user_backends_str && config->user_backends_str[i]; i++) {
            ...
            if (is_complete) {
                if (user_backends_handle(bs, user, backends, ADD_BACKENDS) != 0) {
                    g_log_dbproxy(g_critical, "add user backends failed: user %s doesn't exist", user);
            }
        }
    ...
    if (gerr) {
            g_log_dbproxy(g_critical, "create %s thread failed. %s", pti[i].plugin_thread_names, gerr->message);
            g_error_free(gerr);
            gerr = NULL;
...
#ifdef FILTER_DEBUG
            g_log_dbproxy(g_message, "[filter][reserved query][added] %s", sql_rewrite);
#endif
        }
        g_rw_lock_writer_unlock(&cur_rq->rq_lock);
