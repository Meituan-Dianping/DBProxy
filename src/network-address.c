...
network_address *network_address_new() {
    ...
    addr->hostname = g_string_new(NULL);
    return addr;
}
...
void network_address_free(network_address *addr) {
    ...
    g_string_free(addr->hostname, TRUE);
    g_free(addr);
}
...
static gint network_address_set_address_ip(network_address *addr, const gchar *address, guint port) {
    ...
    if (NULL == address ||
        ...
        if ((rc = getaddrinfo(address, NULL, &hint, &ai)) != 0) {
            g_log_dbproxy(g_warning, "getaddrinfo(%s) failed with %s", address, 
                       gai_strerror(rc));
            return -1;
        }
        ...
        do {
            ...
            if (family == PF_INET) {
                struct in_addr d;
                gchar host[NI_MAXHOST], service[NI_MAXSERV];

                memcpy(&addr->addr.ipv4,
                        (struct sockaddr_in *) ai->ai_addr, 
                            sizeof (addr->addr.ipv4));
                
                if (0 >= addr->hostname->len) {
                    gint err = 0, flags = 0;
                    err = getnameinfo((struct sockaddr *)&(addr->addr.ipv4),
                                        sizeof(struct sockaddr), host, sizeof(host),
                                        service, sizeof(service), flags);
                    if (0 == err ) {
                        if (inet_aton(host, &d) == 0) {
                            g_string_printf(addr->hostname, "%s", host);
                        }
                    }
                }
                break;
...
#endif
        g_log_dbproxy(g_critical, "is-local family %d != %d",
