...
int test_prep_stmt(MYSQL *mysql) {
    ...
    if (NULL == (stmt = mysql_stmt_init(mysql))) {
            g_log_dbproxy(g_critical, "mysql_stmt_init() failed: %s", 
                mysql_error(mysql));
    }

    START_TIMING("%s:%d", mysql->host, mysql->port);
    if (mysql_stmt_prepare(stmt, C(TEST_QUERY_SMALL_PS))) {
           g_log_dbproxy(g_critical, "mysql_stmt_prepare(%s) failed: %s", 
                TEST_QUERY_SMALL_PS,
    ...
    if (mysql_stmt_bind_param(stmt, params)) {
           g_log_dbproxy(g_critical, "mysql_stmt_bind_param() failed: %s", mysql_stmt_error(stmt));
    }
    ...
    for (round = 0; round < 2; round++) {
        ...
        if (mysql_stmt_execute(stmt)) {
                g_log_dbproxy(g_critical, "mysql_stmt_execute() failed: %s", mysql_stmt_error(stmt));
        }
        ...
        if (mysql_stmt_bind_result(stmt, result)) {
                g_log_dbproxy(g_critical, "mysql_stmt_bind_result() failed: %s", mysql_stmt_error(stmt));
        }
...
int test_prep_stmt_noparam(MYSQL *mysql) {
    ...
    if (NULL == (stmt = mysql_stmt_init(mysql))) {
            g_log_dbproxy(g_critical, "mysql_stmt_init() failed: %s", mysql_error(mysql));
    }
...
int test_select(MYSQL *mysql) {
    ...
    for (round = 0; round < 5; round++) {
        ...
        if (mysql_real_query(mysql, C(TEST_QUERY_SMALL))) {
               g_log_dbproxy(g_critical, "mysql_real_query(%s) failed: %s", 
                    TEST_QUERY_SMALL,
                    mysql_error(mysql));
        }
        ...
        if (NULL == (res = mysql_store_result(mysql))) {
            g_log_dbproxy(g_critical, "mysql_store_result() failed: %s", 
                    mysql_error(mysql));
        }
...
int test_server(const gchar *host, guint port, const gchar *user, const gchar *password, const gchar *db) {
    ...
    if (!mysql_real_connect(mysql, host, user, password, db, port, NULL, 0)) {
            g_log_dbproxy(g_critical, "mysql_real_connect() failed: %s", mysql_error(mysql));
    }
