...
int chassis_frontend_options_free(GOptionEntry *entries) {
    ...
    for(i = 0; entries[i].long_name != NULL; i++) {
        gchar *tmp = NULL;
        if (entries[i].long_name) {
            tmp = (gchar *)entries[i].long_name;
            g_free(tmp);
            entries[i].long_name = NULL;
        }
        if (entries[i].description) {
            tmp = (gchar *)entries[i].description;
            g_free(tmp);
            entries[i].description = NULL;
        }
        if (entries[i].arg_description) {
            tmp = (gchar *)entries[i].arg_description;
            g_free(tmp);
            entries[i].arg_description = NULL;
        }
    }
