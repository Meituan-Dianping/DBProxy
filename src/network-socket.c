...
#define SOCK_MSG_HANDLE(log_func, sock, msg)                      \
do {                                                              \
    log_func("%s(%s): event_thread(%d) "                          \
                 "sock->src:%s sock->dst:%s %s %s(%d)",           \
...
network_socket *network_socket_new(network_socket_dir_t socket_dir) {
    network_socket *s;
    ...
    s->conn_attr.default_db = g_string_new(NULL);
    s->conn_attr.charset_client = g_string_new(NULL);
    s->conn_attr.charset_connection = g_string_new(NULL);
    s->conn_attr.charset_results = g_string_new(NULL);
...
void network_socket_free(network_socket *s) {
    ...
    g_string_free(s->conn_attr.default_db, TRUE);
    g_string_free(s->conn_attr.charset_client, TRUE);
    g_string_free(s->conn_attr.charset_connection, TRUE);
    g_string_free(s->conn_attr.charset_results, TRUE);

    g_queue_free_full(s->conn_attr.set_vars, (GDestroyNotify)set_var_free);

    g_free(s);
}

void network_socket_set_chassis(network_socket *s, void *srv)
{
    s->srv = srv;
}
...
#endif
        SOCK_MSG_HANDLE(g_critical, sock, "set_non_blocking() failed");
...
network_socket *network_socket_accept(network_socket *srv) {
    ...
    client = network_socket_new(SOCKET_CLIENT);
...
#endif
        val = 1;
        setsockopt(client->fd, IPPROTO_TCP, TCP_NODELAY, &val, sizeof(val) );
        val = 1;
        setsockopt(client->fd, SOL_SOCKET, SO_KEEPALIVE, &val, sizeof(val) );
...
network_socket_retval_t network_socket_connect_setopts(network_socket *sock) {
    ...
    if (-1 == getsockname(sock->fd, &sock->src->addr.common, &(sock->src->len))) {
        SOCK_MSG_HANDLE(g_warning, sock, "getsockname() failed");
        network_address_reset(sock->src);
    } else if (network_address_refresh_name(sock->src)) {
        SOCK_MSG_HANDLE(g_warning, sock, "network_address_refresh_name failed");
        network_address_reset(sock->src);
    }
...
#endif
        /* getsockopt failed */
        SOCK_MSG_HANDLE(g_critical, sock, "getsockopt() failed");
        return NETWORK_SOCKET_ERROR;
    }
...
network_socket_retval_t network_socket_connect(network_socket *sock) {
    ...
    if (-1 == (sock->fd = socket(sock->dst->addr.common.sa_family, sock->socket_type, 0))) {
        ...
        errno = WSAGetLastError();
#endif
        SOCK_MSG_HANDLE(g_critical, sock, "socket() failed");
        return NETWORK_SOCKET_ERROR;
    }
    ...
    if (-1 == connect(sock->fd, &sock->dst->addr.common, sock->dst->len)) {
        ...
        case E_NET_WOULDBLOCK: /* win32 uses WSAEWOULDBLOCK */
            return NETWORK_SOCKET_ERROR_RETRY;
        default:
            SOCK_MSG_HANDLE(g_critical, sock, "connect() failed");
            return NETWORK_SOCKET_ERROR;
        }
    }
...
network_socket_retval_t network_socket_bind(network_socket * con) {
    ...
    if (con->socket_type == SOCK_STREAM) {
        ...
        if (-1 == (con->fd = socket(con->dst->addr.common.sa_family, con->socket_type, 0))) {
            SOCK_MSG_HANDLE(g_critical, con, "socket() failed");
            return NETWORK_SOCKET_ERROR;
        }
        ...
        if (con->dst->addr.common.sa_family == AF_INET || 
            ...
            if (0 != setsockopt(con->fd, IPPROTO_TCP, TCP_NODELAY, &val, sizeof(val))) {
                SOCK_MSG_HANDLE(g_critical, con, "setsockopt(IPPROTO_TCP, TCP_NODELAY) failed");
                return NETWORK_SOCKET_ERROR;
            }
            
            if (0 != setsockopt(con->fd, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val))) {
                SOCK_MSG_HANDLE(g_critical, con, "setsockopt(SOL_SOCKET, SO_REUSEADDR) failed");
                return NETWORK_SOCKET_ERROR;
            }
        }

        if (-1 == bind(con->fd, &con->dst->addr.common, con->dst->len)) {
            SOCK_MSG_HANDLE(g_critical, con, "bind() failed");
            return NETWORK_SOCKET_ERROR;
        }

        if (-1 == listen(con->fd, 128)) {
            SOCK_MSG_HANDLE(g_critical, con, "listen() failed");
            return NETWORK_SOCKET_ERROR;
        }
    } else {
        ...
        if (-1 == (con->fd = socket(con->src->addr.common.sa_family, con->socket_type, 0))) {
            SOCK_MSG_HANDLE(g_critical, con, "UDP socket() failed");
            return NETWORK_SOCKET_ERROR;
        }

        if (-1 == bind(con->fd, &con->src->addr.common, con->src->len)) {
            SOCK_MSG_HANDLE(g_critical, con, "UDP bind() failed");
            return NETWORK_SOCKET_ERROR;
        }
    }
...
network_socket_retval_t network_socket_read(network_socket *sock) {
    ...
    if (sock->to_read > 0) {
        ...
        network_mysqld_socket_stat(sock->srv, sock->socket_dir, FALSE, len);
    }
...
static network_socket_retval_t network_socket_write_writev(network_socket *con, int send_chunks) {
    ...
    if (-1 == len) {
        ...
        case E_NET_CONNABORTED:
            ...
            return NETWORK_SOCKET_ERROR;
        default:
            SOCK_MSG_HANDLE(g_critical, con, "writev() failed");
            return NETWORK_SOCKET_ERROR;
        }
    } else if (len == 0) {
        return NETWORK_SOCKET_ERROR;
    ...
    network_mysqld_socket_stat(con->srv, con->socket_dir, TRUE, len);
    ...
    for (chunk = con->send_queue->chunks->head; chunk; ) {
        ...
        if (-1 == len) {
            ...
            case E_NET_CONNABORTED:
                ...
                return NETWORK_SOCKET_ERROR;
            default:
                SOCK_MSG_HANDLE(g_critical, con, "send() failed");
                return NETWORK_SOCKET_ERROR;
            }
        } else if (len == 0) {
            return NETWORK_SOCKET_ERROR;
...
network_socket_retval_t network_socket_to_read(network_socket *sock) {
    ...
    if (sock->socket_type == SOCK_DGRAM) {
        ...
        if (0 != getsockopt(sock->fd, SOL_SOCKET, SO_NREAD, &b, &b_len)) {
            SOCK_MSG_HANDLE(g_critical, sock, "getsockopt for SO_NREAD failed");
            return NETWORK_SOCKET_ERROR;
        } else if (b < 0) {
            SOCK_MSG_HANDLE(g_critical, sock, "getsockopt() return negative value");
            return NETWORK_SOCKET_ERROR;
        } else {
            sock->to_read = b;
            return NETWORK_SOCKET_SUCCESS;
    ...
    if (0 != ioctl(sock->fd, FIONREAD, &b)) {
        SOCK_MSG_HANDLE(g_critical, sock, "ioctl(FIONREAD) failed");

        return NETWORK_SOCKET_ERROR;
    } else if (b < 0) {
        SOCK_MSG_HANDLE(g_critical, sock, "ioctl(FIONREAD) return negative bytes to read");
        return NETWORK_SOCKET_ERROR;
    } else {
        sock->to_read = b;
        return NETWORK_SOCKET_SUCCESS;
