...
*/
    ...
    /*check the blacklist file can read or write, and the black list file dir can execute and write*/
    gchar *blacklist_file = srv->proxy_filter->blacklist_file;
    gchar *blacklist_file_dir = g_path_get_dirname(blacklist_file);

    if (-1 == faccessat(0, blacklist_file, R_OK | W_OK, AT_EACCESS)
            || -1 == faccessat(0, blacklist_file_dir, X_OK | W_OK, AT_EACCESS)) {
        g_free(blacklist_file_dir);
        g_warning("%s(%s): %s don't have correct privilege for the user %s set in the blacklist file, blacklist file should have read write privileges and the blacklist file dir should have execute and write privilege.", G_STRLOC, __func__, blacklist_file, srv->user);
    }
    g_free(blacklist_file_dir);
