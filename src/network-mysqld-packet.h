...
NETWORK_API GString* network_mysqld_auth_challenge_dump(network_mysqld_auth_challenge *shake);
NETWORK_API GString* network_mysqld_auth_response_dump(network_mysqld_auth_response *auth);

#define CON_MSG_HANDLE_INTERNAL(log_func, con, msg, strloc, func)             \
do {                                                                          \
    g_assert(msg != NULL);                                                    \
                                                                              \
    if (con->server && con->server->dst != NULL && con->server->dst->name->len > 0) {                \
    log_func("%s(%s): event_thread(%d) C:%s C_usr:%s S:%s(thread_id:%u) S_usr:%s Db:%s %s %s",          \
                strloc, func, chassis_event_get_threadid(),                         \
                ...
                network_mysqld_conn_stat_desc[con->state], msg);                    \
    } else {                                                                        \
        log_func("%s(%s): event_thread(%d) C:%s C_usr:%s %s %s",                    \
                strloc, func, chassis_event_get_threadid(),                         \
                NETWORK_SOCKET_SRC_NAME(con->client),                               \
                NETWORK_SOCKET_USR_NAME(con->client),                               \
                network_mysqld_conn_stat_desc[con->state], msg);                    \
...
#define CON_MSG_HANDLE(log_func, con, msg)  CON_MSG_HANDLE_INTERNAL(log_func, \
                                                        con, msg, G_STRLOC, __func__)
